...
void WaveThetaSerial::setup_system()
{
  std::cout << "===============================================" << std::endl;
  std::cout << "Setting up the system" << std::endl;
  std::cout << "Test case: " << WaveEquation::test_case_name(test_case)
            << std::endl;
...
void WaveThetaSerial::output_results() const
{
...
double WaveThetaSerial::compute_error(const VectorTools::NormType &norm_type,
...
double WaveThetaSerial::compute_total_energy() const
{
      ...
      if (exact_solution != nullptr && timestep_number % error_interval == 0)
        {
          ...
          std::cout << "   Time = " << time << ", L2 error = " << error_L2
